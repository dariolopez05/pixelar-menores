...
set MODEL_URL=https://github.com/dariolopez05/pixelar-menores/releases/download/v1.0-model/resnet18_age_detection.pt
set MODEL_DIR=%~dp0..\age-detection\model

if not exist "%MODEL_DIR%" mkdir "%MODEL_DIR%"

if exist "%MODEL_DIR%\resnet18_age_detection.pt" (
    echo [OK] El modelo ya existe en age-detection\model\resnet18_age_detection.pt
    echo      Borra el archivo manualmente si quieres descargarlo de nuevo.
    exit /b 0
)

echo Descargando modelo desde GitHub Releases...
curl -L -o "%MODEL_DIR%\resnet18_age_detection.pt" "%MODEL_URL%"
...
echo [OK] Modelo descargado en age-detection\model\resnet18_age_detection.pt
