#!/usr/bin/env python3
"""
Clasificador binario menor/adulto — MobileNetV2 + transfer learning v2.
Dataset: dataset/face_age/ con carpetas nombradas con la edad (p.ej. "017"),
         imágenes PNG dentro de cada carpeta.
Mejoras sobre v1:
  - Aumento de datos más agresivo: rotación, zoom, traslación, volteo vertical,
    brillo, contraste, saturación, tono.
  - Capas de augmentación integradas en el pipeline de tf.data.
...
# Nuevo dataset: dataset/face_age/  (carpetas 001, 002, …)
DATASET_DIR = Path(os.environ.get("DATASET_DIR", str(_root / "dataset" / "face_age")))
...
VAL_SPLIT         = 0.2
SEED              = 42
MAX_IMGS_PER_AGE  = 120          # cap general para evitar dominio de una sola edad
AGE_1_CAP         = 50           # edad 1 tiene ~1112 imgs → cap muy agresivo
BORDERLINE_AGES   = set(range(13, 18))   # edades críticas cerca de la frontera
...
def load_raw_dataset(dataset_dir: Path):
    ...
    for age_dir in sorted(dataset_dir.iterdir()):
        ...
        # Acepta PNG y JPG
        imgs = list(age_dir.glob("*.png")) + list(age_dir.glob("*.jpg")) + list(age_dir.glob("*.jpeg"))
...
def oversample_borderline(paths, labels, ages):
    ...
    if extra_p:
        ...
        log.info(
            "Oversampling borderline %s: +%d imgs → %d total",
            sorted(BORDERLINE_AGES), len(extra_p), len(paths),
        )
    return paths, labels
...
def preprocess(path):
    ...
    # decode_image acepta PNG y JPG sin especificar formato
...
def augment(img):
    """
    Aumento de datos aplicado solo al conjunto de entrenamiento.
    Aplica transformaciones geométricas y de color de forma aleatoria.
    Las operaciones geométricas usan tf.keras.layers en modo eager;
    las de color usan tf.image directamente sobre el tensor [-1, 1].
    """
    # ── Geométricas ───────────────────────────────────────────────────────────
    img = tf.image.random_flip_left_right(img)
    img = tf.image.random_flip_up_down(img)   # volteo vertical (poco frecuente pero útil)

    # Rotación aleatoria ±15° (0.1 * 2π ≈ 36°, usamos factor 0.083 ≈ ±15°)
    img = tf.expand_dims(img, 0)   # [1, H, W, 3]
    img = tf.keras.layers.RandomRotation(
        factor=0.083, fill_mode="reflect", seed=None
    )(img, training=True)

    # Zoom aleatorio: acercar/alejar entre 10%
    img = tf.keras.layers.RandomZoom(
        height_factor=(-0.10, 0.10), width_factor=(-0.10, 0.10),
        fill_mode="reflect", seed=None
    )(img, training=True)

    # Traslación aleatoria: hasta 10% en cada eje
    img = tf.keras.layers.RandomTranslation(
        height_factor=0.10, width_factor=0.10,
        fill_mode="reflect", seed=None
    )(img, training=True)
    ...
    # ── Color ─────────────────────────────────────────────────────────────────
    # tf.image opera sobre [0,1] pero los valores ya están en [-1,1];
    # las operaciones de brillo/contraste son aditivas/multiplicativas
    # y funcionan igual en ese rango. Recortamos al final.
    img = tf.image.random_brightness(img, max_delta=0.30)
    img = tf.image.random_contrast(img, lower=0.70, upper=1.40)
    img = tf.image.random_saturation(img, lower=0.70, upper=1.40)
...
def build_model(trainable_base=False):
    ...
    return models.Model(inp, out, name="age_classifier_v2")
...
def plot_history(h1, h2, out_dir: Path):
    ...
    for ax, (train_m, val_m), title in zip(
        ...
        ax.legend()
    fig.tight_layout()
    fig.savefig(out_dir / "training_curves_v2.png", dpi=120)
    plt.close(fig)
...
def main():
    ...
    if not DATASET_DIR.exists():
        log.error("Dataset no encontrado: %s", DATASET_DIR)
        sys.exit(1)
    ...
    # Focal Loss: focaliza el aprendizaje en ejemplos difíciles (teens 15-17)
    ...
    h2 = model.fit(
        ...
        callbacks=[
            ...
            callbacks.ModelCheckpoint(
                str(MODEL_PATH), monitor="val_recall", mode="max",
    ...
    report = {
        ...
        "augmentations":                      [
            ...
            "random_zoom_10pct",
            "random_translation_10pct",
            "random_brightness_0.30",
            "random_contrast_0.70-1.40",
            "random_saturation_0.70-1.40",
            "random_hue_0.10",
        ],
    ...
    (REPORTS_DIR / "training_report_v2.json").write_text(json_str)
